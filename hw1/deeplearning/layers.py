...
def batchnorm_backward(dout, cache):
    ...
    dz6 = gamma*dz7
    dz5 = (z1*dz6).sum(axis=0)
    dz4 = dz5*(-1/z4**2)
    dz3 = dz4*0.5/np.sqrt(z3+epsilon)
    dz2 = 1/N*np.ones((N,D))*dz3
    dz1 = dz6*z5+2*z1*dz2
    dz0 = -dz1.sum(axis=0)
    dx = 1/N * np.ones((N,D)) * dz0 + dz1
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    xp = np.pad(x,npad,mode="constant")
    for n in range(N):
        for f in range(F):
            xmat = xp[n, :, :, :]
            wmat = w[f, :, :, :]
...
def max_pool_forward_naive(x, pool_param):
    ...
    HH = int((H-hp)/stride + 1)
    WW = int((W-wp)/stride + 1)
    ...
    for n in range(N):
        for c in range(C):
            for h in range(HH):
                for w in range(WW):
                    out[n,c,h,w] = np.max(x[n, c, h*stride:h*stride + hp, w*stride:w*stride + wp])
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(HH):
                for w in range(WW):
                    out = np.max(x[n, c, h*stride:h*stride + hp, w*stride:w*stride + wp])
                    for h_ in range(h*stride, h*stride + hp):
                        for w_ in range(w*stride, w*stride + wp):
                            if x[n,c,h_,w_] == out:
                                dx[n,c,h_,w_] = dout[n,c,h,w]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
...
def spatial_batchnorm_backward(dout, cache):
    ...
    """
    tmp = dout
    dx, dgamma, dbeta = batchnorm_backward(dout.reshape([-1,tmp.shape[1]]),cache)
